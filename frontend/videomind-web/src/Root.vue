...
onMounted(async () => {
  try {
    ...
    await loadBinding()
    if (new URLSearchParams(location.search).get('mindagent') === 'bound') {
...
<template>
  ...
  <template v-else>
    <div class="account-toolbar">
      ...
      <button :class="{ bound: binding.bound }" @click="bind">
        {{ binding.bound ? `已绑定 ${binding.username || 'MindAgent'}` : '绑定 MindAgent' }}
      </button>
